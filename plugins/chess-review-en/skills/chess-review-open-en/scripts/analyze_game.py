...
def main():
    ...
    try:
        ...
        def evaluate(board,seconds):
            ...
            return {'fen':board.fen(),'cp':score.score(),'mate':score.mate(),'rank_score':score.score(mate_score=100000),
        ...
        for i,move in enumerate(moves):
            ...
            rows.append({'ply':i,'move':label(boards[i],move),'side':'white' if boards[i].turn else 'black',
                         'is_user':boards[i].turn==user_color,'uci':move.uci(),'before':before,'after':after,'loss_rank':loss})
        candidates=[]
        for side in ('white','black'):
            sign=1 if (side=='white')==user_color else -1
            candidates.extend(sorted((r for r in rows if r['side']==side),key=lambda r:sign*r['loss_rank'],reverse=True)[:max(0,args.refine_count)])
        ...
        data={'source_pgn_sha256':hashlib.sha256(pgn.encode()).hexdigest(),'engine':engine.id,'perspective':args.perspective,'screen_seconds':args.seconds,'refine_seconds':args.refine_seconds,
